...
describe("Condominium", function () {
  ...
  it("Should NOT add topic (duplicated)", async function () {
    const { contract, manager, accounts } = await loadFixture(deployFixture);
    await contract.addTopic("topic 1", "description 1", Category.DECISION, 0, manager.address);
    await expect(contract.addTopic("topic 1", "description 1", Category.DECISION, 0, manager.address)).to.be.revertedWith("The topic does not exists");
  });
  ...
  it("Should NOT vote (empty)", async function () {
    ...
    await expect(instance.vote("topic 1", Options.EMPTY))
      .to.be.revertedWith("The option can not be EMPTY");
  });
  ...
  it("Should NOT close voting (minimum votes)", async function () {
    ...
    await expect(contract.closeVoting("topic 1"))
      .to.be.revertedWith("You can not finish a voting without a minimum votes");
  });
  ...
  it("Should NOT open voting (status)", async function () {
    ...
    await expect(contract.openVoting("topic 1"))
      .to.be.revertedWith("Only IDLE topics can be open for votes");
  });
  ...
  it("Should NOT transfer (topic)", async function () {
    ...
    await expect(contract.transfer("topic 1", 100))
      .to.be.revertedWith("Only APPROVED SPENT topics can be used for transfer");
  });
