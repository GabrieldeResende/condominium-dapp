...
contract Condominium is ICondominium {
    ...
    function addTopic(
        ...
        require(!topicExists(title), "The topic does not exists");
        if (amount > 0) {
            require(
    ...
    function openVoting(
        ...
        require(
            topic.status == Lib.Status.IDLE,
            "Only IDLE topics can be open for votes"
        );
    ...
    function closeVoting(
        ...
        require(
            numberOfVotes(title) >= minimuVotes,
            "You can not finish a voting without a minimum votes"
        );
    ...
    function transfer(
        ...
        require(
            ...
            "Only APPROVED SPENT topics can be used for transfer"
        );
        require(
            topic.amount >= amount,
